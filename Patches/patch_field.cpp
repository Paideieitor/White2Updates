...
#if ADD_INFINITE_REPEL
ScriptWork* EventScriptCall_Start(GameEvent* parentEvent, u16 scrId, FieldActor* actor, int a4, HeapID heapId);
int EventFieldToggleRepel_Callback(GameEvent* gEvent, u32* pState, EventFieldToggle* data)
{
    ...
    if (!encountSave->RepelSteps)
    {
        ...
        EventScriptCall_Start(gEvent, 10144, 0, 0, HEAPID_FIELDMAP);
    }
    else
    {
        DPRINT("DEACTIVATED INFINITE REPEL\n");
        encountSave->RepelSteps = 0x00;
        EventScriptCall_Start(gEvent, 10130, 0, 0, HEAPID_FIELDMAP);
    }
...
GameEvent* EventFieldToggleRepel_Create(Field* field, GameSystem* gsys)
{
    GameEvent* repelEvent;
    int* repelEventData;

    repelEvent = GameEvent_Create(gsys, 0, (GameEventCallback)EventFieldToggleRepel_Callback, 8u);
    repelEventData = (int*)GameEvent_GetData(repelEvent);
    repelEventData[0] = (int)gsys;
    repelEventData[1] = (int)field;
